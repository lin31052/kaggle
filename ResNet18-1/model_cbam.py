"""ResNet18 + CBAM 注意力（Convolutional Block Attention Module）

与 model.py 的 ResNet18 结构完全一致，仅在每个残差块内、
残差分支相加之前插入 CBAM（通道注意力 + 空间注意力）。
"""
import torch
...
class ResidualCBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.cbam(y)  # 通道 + 空间注意力加权后，再与恒等映射相加
        if self.conv3:
            x = self.conv3(x)
        y = self.ReLU(y + x)
        return y
...
if __name__ == "__main__":
    ...
    model = ResNet18CBAM(ResidualCBAM).to(device)
    print(model)
